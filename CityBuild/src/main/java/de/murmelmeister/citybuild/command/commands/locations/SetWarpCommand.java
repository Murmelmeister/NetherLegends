...
public class SetWarpCommand extends CommandManager {
    public SetWarpCommand(CityBuild plugin) {
        super(plugin);
    }
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        if (!(isEnable(sender, Configs.COMMAND_ENABLE_SET_LOCATION))) return true;
        if (!(hasPermission(sender, Configs.PERMISSION_SET_LOCATION))) return true;
